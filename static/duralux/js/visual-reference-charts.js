(() => {
  ...
  const dateFormatter = new Intl.DateTimeFormat(documentLanguage, {
    day: "2-digit",
    month: "short",
    year: "numeric",
    timeZone: "UTC",
  ...
  const optionsFor = (element) => {
    ...
    const categories = [
      element.getAttribute("data-category-1"),
      element.getAttribute("data-category-2"),
      element.getAttribute("data-category-3"),
    ].filter(Boolean);
    ...
    return {
      ...
      colors: [colors.brand],
      series: [{ name: element.dataset.seriesLabel, data: values }],
      ...
      xaxis: {
        ...
        title: { text: element.dataset.axisLabel },
      },
      yaxis: {
        min: 0,
        forceNiceScale: true,
        title: { text: element.dataset.valueLabel },
      },
  ...
  const initialize = (element) => {
    ...
    element.setAttribute("aria-label", element.dataset.chartLabel);
