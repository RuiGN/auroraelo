...
def test_therapist_dashboard_uses_local_progressive_chart_enhancement() -> None:
    ...
    assert (
        "<caption>Cadastros por mês</caption>" in source
        or '<caption>{% translate "Cadastros por mês" %}</caption>' in source
    )
    table_classes = re.findall(r'<table\b[^>]*class="([^"]+)"', source)
    assert any(
        {"table", "product-table"} <= set(value.split()) for value in table_classes
    )
