...
def test_mobile_drawer_has_focus_escape_overlay_and_scroll_lock_contract(
    ...
    assert 'aria-modal="true"' not in content
...
def test_mobile_drawer_closes_when_viewport_enters_desktop_mode() -> None:
    ...
    assert "closeSidebar(false);" in script
    assert "sidebar.inert = !desktopLayout.matches" in script
...
def test_product_shell_supports_accessible_local_text_copy() -> None:
    ...
    assert "button.dataset.copySuccess" in script
    assert "button.dataset.copyFailure" in script
    assert 'status.textContent = "Chave copiada."' not in script
...
def test_sprint_three_account_and_error_templates_use_duralux_foundation() -> None:
    ...
    title_fallback = (
        "{% if page_title %}{{ page_title }}{% else %}{{ title }}{% endif %}"
    )
