...
def test_recommendation_notification_uses_each_patient_language() -> None:
    clinic, _admin = _clinic_with_admin()
    therapist = _therapist(clinic, verified=True)
    patient_en = _patient(clinic)
    patient_es = _patient(clinic)
    patient_en.preferred_language = "en"
    patient_en.save(update_fields=("preferred_language",))
    patient_es.preferred_language = "es"
    patient_es.save(update_fields=("preferred_language",))
    content = _published_content(clinic, therapist, "idioma-notificacao")

    content_services.recommend_content(
        clinic_id=clinic.pk,
        actor=therapist,
        content_id=content.pk,
        patient_id=patient_en.pk,
        cohort_id=None,
        objective="English",
        priority="normal",
    ...
    content_services.recommend_content(
        ...
        actor=therapist,
        content_id=content.pk,
        patient_id=patient_es.pk,
        cohort_id=None,
        objective="Español",
        priority="normal",
        context="Prueba de idioma.",
        valid_days=30,
        request_id=uuid4(),
    )
    ...
    english = ContentNotification.infrastructure_objects.get(
        clinic_id=clinic.pk, recipient_id=patient_en.pk
    )
    spanish = ContentNotification.infrastructure_objects.get(
        clinic_id=clinic.pk, recipient_id=patient_es.pk
    )
    assert english.body == "A content recommendation was assigned to you."
    assert spanish.body == "Se te ha asignado una recomendación de contenido."
