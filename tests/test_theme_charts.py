...
def test_theme_bootstrap_runs_before_styles_and_theme_is_pinned(
    client: Client,
) -> None:
    _login(client)
...
def test_theme_script_pins_light_identity_without_switch() -> None:
    ...
    assert 'safeStorageGet("product-theme")' not in script
    assert "data-theme-toggle" not in script
