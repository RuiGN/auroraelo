...
def test_mindcare_brand_replaces_source_identity_in_runtime() -> None:
    ...
    brand = (templates / "layouts/partials/brand.html").read_text()
    assert "active_clinic_branding.logo.url" in brand
    assert "active_clinic_branding.display_name" in brand
...
def test_duralux_bootstrap_bundle_includes_popper_and_its_notice() -> None:
    ...
    assert "@popperjs/core@2.11.8" in (
        Path(settings.BASE_DIR) / "docs" / "duralux" / "runtime-asset-manifest.md"
    ).read_text(encoding="utf-8")
...
def test_font_namespace_is_deferred_and_vendor_is_licensed_and_scoped() -> None:
    ...
    assert not list((Path(settings.BASE_DIR) / "static" / "duralux").rglob(".gitkeep"))
...
def test_documented_runtime_hashes_match_the_published_assets() -> None:
    manifest_path = Path(settings.BASE_DIR) / "docs" / "migration" / "runtime-assets.md"
    manifest = manifest_path.read_text(encoding="utf-8")
    documented = dict(re.findall(r"\| `([^`]+)` \| `([0-9a-f]{64})` \|", manifest))
    runtime_root = Path(settings.BASE_DIR) / "static" / "duralux"

    assert set(documented) == {
        path.removeprefix("duralux/") for path in DURALUX_RUNTIME_FILES
    }
