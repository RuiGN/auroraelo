...
def test_form_component_uses_duralux_bootstrap_classes() -> None:
    ...
    assert 'class="product-form-grid"' in content
    assert 'field-span--sm' in content
    assert 'field-span--lg' in content
...
def test_form_guards_duplicate_unsaved_and_destructive_actions() -> None:
    ...
    assert "canonicalValueFor(input)" in script
    assert "canonicalize(input).slice(0, maxDigits)" not in script
...
def test_form_script_focuses_first_invalid_field_and_recovers_from_bfcache() -> None:
    ...
    assert (
        '(invalidField || document.querySelector("[data-focus-error-summary]"))'
        "?.focus()" in script
    )
