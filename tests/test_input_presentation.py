...
def test_textual_controls_receive_decorative_icon(field, icon):
    field = bound(field)
    assert field_icon(field) == icon
    html = render_to_string("components/duralux_field.html", {"field": field})
...
def test_longer_short_values_are_not_squeezed_into_quarter_column(field):
    assert (
        field_span(
...
def test_time_input_uses_native_picker_and_canonical_initial():
...
def test_explicit_presentation_and_form_value_are_preserved():
    field = bound(
        forms.CharField(
...
def test_generic_text_hint_is_translatable_label_not_an_example_value():
...
def test_decimal_hint_matches_active_locale(language, hint):
...
def test_date_value_remains_iso_and_has_no_text_mask():
