...
class GoalForm(forms.Form):
    ...
    title = forms.CharField(
        label=_("Qual é a sua meta?"),
        max_length=255,
        help_text=_("Ex.: Retomar caminhadas diárias"),
    )
    description = forms.CharField(
        label=_("Detalhes (opcional)"),
        max_length=4000,
        required=False,
        widget=forms.Textarea(attrs={"rows": 3}),
        help_text=_("Opcional. Máximo de 4000 caracteres."),
    )
    horizon = forms.ChoiceField(
        label=_("Horizonte de tempo"),
    ...
    due_date = forms.DateField(
        label=_("Prazo (opcional)"),
        required=False,
        widget=forms.DateInput(attrs={"type": "date"}),
        help_text=_(
            "Você pode alterar ou remover o prazo quando quiser, sem penalidade."
        ),
    )
    steps_raw = forms.CharField(
        label=_("Pequenas etapas"),
        required=False,
    ...
    def clean_title(self) -> str:
        ...
        if not value:
            raise forms.ValidationError(_("Dê um título à sua meta."))
        return value
    ...
    def clean_steps_raw(self) -> list[str]:
        ...
        if len(steps) > 50:
            raise forms.ValidationError(_("Uma meta pode ter no máximo 50 etapas."))
        return steps
