...
|---|---|---|---|---|---|
| `accounts/auth_form.html` | `/accounts/login/` `account_login`; convite, recuperação/reset | anônimo nos fluxos públicos | `accounts/auth_base.html` | `auth-login-minimal`, `auth-register-minimal`, `auth-reset-minimal`, `auth-resetting-minimal` | 3 |
...
| Template Django | Rota / view consumidora | Perfil autorizado | Layout pai | Referência Duralux | Sprint |
|---|---|---|---|---|---|
| `accounts/auth_base.html` | base de autenticação e erros | herda o consumidor público/autenticado | HTML completo | família `auth-*-minimal` | 3 |
...
| `components/duralux_control.html` | controles de formulários com rótulos específicos | herda o consumidor | partial sem layout próprio | controles com ícones | transversal |
| `components/language_selector.html` | seletor de idioma dos shells público e autenticado | herda o consumidor | partial sem layout próprio | cabeçalho Duralux | transversal |
| `layouts/partials/brand.html` | marca dos layouts | herda o consumidor | partial sem layout próprio | marca Duralux | transversal |
| `workspace/shortcut.html` | atalhos do workspace | membro autorizado pelo consumidor | layout do workspace | workspace Duralux | transversal |
