...
_PURPOSE_LABELS: dict[ConsentPurpose, str] = {
    ConsentPurpose.TERMS_OF_USE: gettext_noop("Termos de uso"),
    ConsentPurpose.CLINICAL_LIMITS: gettext_noop("Limites do atendimento"),
    ConsentPurpose.CLINICAL_FOLLOW_UP: gettext_noop("Acompanhamento clínico"),
    ConsentPurpose.COMMUNICATION: gettext_noop("Comunicação"),
    ConsentPurpose.STAFF_OPERATIONS: gettext_noop("Operação da equipe"),
}
...
def purpose_label_pt_br(purpose: ConsentPurpose | str) -> str:
    """Translate at request time while keeping the existing public helper name."""
    return gettext(_PURPOSE_LABELS[ConsentPurpose(purpose)])
