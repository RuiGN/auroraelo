    ...
    *,
    ...
    if not has_active_clinic_role(
        ...
        raise PermissionDenied(
            _("Somente a administração pode reconhecer esta tarefa.")
        )
    reference = acknowledgement_reference.strip()
    if not reference:
        raise ValidationError(_("Informe a referência do atendimento operacional."))
    work_item = (
    ...
    if work_item is None:
        raise ValidationError(_("Tarefa operacional indisponível."))
...
def process_revocation_dispatch(
    *, clinic_id: UUID, dispatch_id: UUID
    ...
    if dispatch is None:
        raise ValidationError(_("Despacho de revogação indisponível."))
...
def _actor_id(actor: AbstractBaseUser) -> UUID:
    ...
    if not isinstance(actor_id, UUID) or not actor.is_active:
        raise PermissionDenied(_("É necessário possuir acesso ativo à clínica."))
    return actor_id
...
def _record_consent_manifestation(
    *,
    ...
    if not subject_has_active_clinic_relationship(
        ...
        raise PermissionDenied(_("Titular sem vínculo ativo com esta clínica."))
    if representation_reference:
        raise PermissionDenied(
            _("Referências textuais não comprovam representação legal.")
        )
    if decision not in (
    ...
    now = timezone.now()
    if document is None:
        raise ValidationError(_("Documento indisponível ou ainda não vigente."))
    ...
    if document.audience not in _audiences_for_roles(subject_roles):
        raise PermissionDenied(_("Documento não destinado a este titular."))
    representation = None
    ...
    if replay is not None:
        ...
        raise ValidationError(_("A chave idempotente já foi usada com outra decisão."))
    latest = (
    ...
    if (
        ...
        raise ValidationError(
            _("Uma autorização aceita só pode ser retirada pelo fluxo de revogação.")
        )
    if latest is not None and latest.decision == ConsentManifestation.Decision.REVOKED:
        raise ValidationError(
            _("Esta versão foi revogada e não pode ser reativada por uma nova decisão.")
        )
...
def revoke_consent(
    *,
    ...
    if actor_id != subject_id or not _active_roles(
        ...
        raise PermissionDenied(
            _("Somente o titular ativo pode revogar esta autorização.")
        )
    normalized_reason = reason.strip()
    if not normalized_reason:
        raise ValidationError(_("Informe o motivo da revogação."))
    lock_clinic_for_update(clinic_id=clinic_id)
    document = ConsentDocument.infrastructure_objects.filter(
        pk=document_id,
        clinic_id=clinic_id,
    ...
    if document is None:
        raise ValidationError(_("Documento indisponível para revogação."))
    require_document_integrity(document)
    if document.is_mandatory:
        raise ValidationError(
            _("Este documento não é uma autorização opcional revogável neste fluxo.")
        )
    replay = ConsentManifestation.infrastructure_objects.filter(
        clinic_id=clinic_id,
        request_id=request_id,
    ...
    if latest is None or latest.decision != ConsentManifestation.Decision.ACCEPTED:
        raise ValidationError(_("Não existe autorização vigente para revogar."))
    now = timezone.now()
    ...
    if not destinations:
        raise ValidationError(_("Nenhum destino de revogação está configurado."))
...
def resolve_purpose_access(
    *,
    ...
    if not roles:
        return PurposeAccess(False, _("Acesso à clínica indisponível."))
    if purpose in _BASIC_RIGHT_PURPOSES:
        return PurposeAccess(
            True, _("Direito básico disponível independentemente de consentimento.")
        )
    now = timezone.now()
    candidates = list(
    ...
    for document in documents:
        ...
        if manifestation is None or manifestation.decision != "accepted":
            return PurposeAccess(
                False,
                _(
                    "%(consequence)s Alternativa: %(alternative)s "
                    "Contato da clínica: %(contact)s Seus direitos básicos "
                    "permanecem acessíveis."
                )
                % {
                    "consequence": document.refusal_consequence,
                    "alternative": document.alternative_instructions,
                    "contact": document.clinic_contact_instructions,
                },
                document.pk,
            )
    return PurposeAccess(True, _("Finalidade autorizada."), documents[-1].pk)
