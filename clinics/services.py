...
def selected_clinic_id(request: HttpRequest) -> UUID:
    """Parse the untrusted header, or the explicit session key, as a UUID.

    The caller must have placed ``CLINIC_SESSION_KEY`` in the session through an
    explicit selection action (login, clinic-switch).  Silent auto-resolution is
    intentionally absent: a user who belongs to multiple clinics must always
    choose one explicitly — resolving it automatically would silently violate the
...
def update_professional_membership(
    *,
    ...
    if role not in {
        ...
        raise ValidationError(gettext("Selecione um papel profissional válido."))
    if valid_until is not None and valid_until < valid_from:
        raise ValidationError(
            gettext("A data final não pode ser anterior à data inicial.")
        )
    membership = (
...
def update_clinic_identity(
    *,
    ...
    if any(not values[field] for field in required):
        raise ValidationError(gettext("Preencha os dados institucionais obrigatórios."))
...
def update_clinic_operations(
    *,
    ...
    try:
        ...
        raise ValidationError(
            gettext("Selecione um fuso horário IANA válido.")
        ) from exc
    if language_code not in {"pt-BR", "en-US"}:
        raise ValidationError(gettext("Selecione um idioma suportado."))
    ...
    if (
        ...
        raise ValidationError(
            gettext("Selecione canais de atendimento válidos e únicos.")
        )
    expected_days = {
        "monday",
        "tuesday",
    ...
    if set(weekly_hours) != expected_days:
        raise ValidationError(
            gettext("Informe os horários de todos os dias da semana.")
        )
    ...
    for day, intervals in weekly_hours.items():
        ...
        for interval in sorted(intervals, key=lambda item: item.get("start", "")):
            if set(interval) != {"start", "end"}:
                raise ValidationError(
                    gettext("Informe início e fim para cada intervalo.")
                )
            try:
                ...
                raise ValidationError(
                    gettext("Use horários no formato HH:MM.")
                ) from exc
            if start >= end or (previous_end is not None and start < previous_end):
                raise ValidationError(
                    gettext("Os intervalos devem ser válidos e não sobrepostos.")
                )
    ...
    if len(instructions) > 1000:
        raise ValidationError(
            gettext("As orientações fora do horário são muito extensas.")
        )
    try:
        configuration = (
        ...
        raise ValidationError(
            gettext("Conclua primeiro os dados institucionais.")
        ) from exc
...
def update_clinic_branding(
    *,
    ...
    if not color_pattern.fullmatch(primary_color) or not color_pattern.fullmatch(
        ...
        raise ValidationError(gettext("Use cores no formato hexadecimal #RRGGBB."))
    primary = primary_color.upper()
    secondary = secondary_color.upper()
    if _contrast_ratio(primary, "#FFFFFF") < 4.5:
        raise ValidationError(
            gettext("A cor primária não tem contraste suficiente no tema claro.")
        )
    if _contrast_ratio(secondary, "#111827") < 4.5:
        raise ValidationError(
            gettext("A cor secundária não tem contraste suficiente no tema escuro.")
        )
    if (logo.size or 0) > 2 * 1024 * 1024:
        raise ValidationError(gettext("O logotipo excede o limite de 2 MB."))
    metadata = PrivateUploadPolicy().validate(logo)
    if metadata.detected_media_type not in {"image/png", "image/jpeg"}:
        raise ValidationError(gettext("O logotipo deve ser PNG ou JPEG."))
...
def update_clinic_modules(
    *,
    ...
    if len(selected) != len(enabled_modules):
        raise ValidationError(gettext("Não repita módulos na configuração."))
    unknown = selected - CLINIC_MODULE_PREREQUISITES.keys()
    if unknown:
        raise ValidationError(gettext("A configuração contém módulos desconhecidos."))
    missing = {
        prerequisite
    ...
    if missing:
        raise ValidationError(
            gettext(
                "Ative os pré-requisitos antes dos módulos dependentes: %(modules)s"
            ),
            params={"modules": ", ".join(sorted(missing))},
        )
    try:
        configuration = (
            ClinicConfiguration.infrastructure_objects.select_for_update().get(
