...
SETUP_STAGES = (
    ("identity", _("Dados institucionais")),
    ("operations", _("Atendimento e horários")),
    ("branding", _("Identidade visual")),
    ("modules", _("Módulos")),
    ("review", _("Revisão")),
)
...
TLS_STATUS_LABELS = {
    "pending": _("Pendente"),
    "active": _("Ativo"),
    "renewal_due": _("Renovação pendente"),
    "failed": _("Falhou"),
}


class _DomainPresentationSource(Protocol):
    domain: str
    status: str
    tls_status: str


def _module_presentations(codes: Iterable[str]) -> tuple[dict[str, object], ...]:
    return tuple(
        {"code": code, "label": MODULE_LABELS.get(code, code)} for code in codes
    )


def _domain_presentations(
    domains: Iterable[_DomainPresentationSource],
) -> tuple[dict[str, object], ...]:
    return tuple(
        {
            "domain": domain,
            "status_label": DOMAIN_STATUS_LABELS.get(domain.status, domain.status),
            "tls_status_label": TLS_STATUS_LABELS.get(
                domain.tls_status, domain.tls_status
            ),
        }
...
def clinic_setup(request: HttpRequest) -> HttpResponse:
    ...
    return TemplateResponse(
        ...
        "clinics/setup.html",
        {
            ...
            "form": form,
            "review_modules": _module_presentations(
                configuration.enabled_modules if configuration else ()
            ),
        },
    )
...
def whitelabel_domains(request: HttpRequest) -> HttpResponse:
    ...
    return TemplateResponse(
        ...
        "clinics/whitelabel_domains.html",
        {
            "layout_template": "layouts/vertical.html",
            "domains": domains,
            "domain_presentations": _domain_presentations(domains),
        },
    )
