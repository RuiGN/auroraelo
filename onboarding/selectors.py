...
def clinic_onboarding_checklist(
    *, clinic_id: UUID, actor: AbstractBaseUser
    ...
    return (
        ClinicOnboardingChecklistItem(
            key="clinic_profile",
            label=_("Perfil da clínica completo"),
            is_complete=clinic_setup_complete(clinic_id=clinic_id),
            detail=_("Identidade, operação, identidade visual e módulos configurados."),
        ),
        ClinicOnboardingChecklistItem(
            key="professionals_invited",
            label=_("Profissionais convidados"),
            is_complete=has_non_patient_memberships(clinic_id=clinic_id),
            detail=_("Pelo menos um profissional ou membro administrativo associado."),
        ),
        ClinicOnboardingChecklistItem(
            key="terms_published",
            label=_("Termos publicados"),
            is_complete=has_published_documents(clinic_id=clinic_id),
            detail=_("Documentos de consentimento publicados e vigentes."),
        ),
        ClinicOnboardingChecklistItem(
            key="permissions_configured",
            label=_("Permissões configuradas"),
            is_complete=has_clinic_admin_membership(clinic_id=clinic_id),
            detail=_("Administrador da clínica definido."),
        ),
        ClinicOnboardingChecklistItem(
            key="first_patient",
            label=_("Primeiro paciente"),
            is_complete=has_patient_profiles(clinic_id=clinic_id),
            detail=_("Pelo menos um paciente cadastrado ou convidado."),
        ),
    )
