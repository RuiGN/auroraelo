...
class PrivateUploadPolicy:
    ...
    def validate(self, upload: UploadedFile) -> PrivateUploadMetadata:
        ...
        if expected is None:
            raise ValidationError(gettext("Tipo de arquivo não permitido."))
        expected_media_type, signatures = expected
        if upload.content_type != expected_media_type:
            raise ValidationError(gettext("O tipo declarado do arquivo é inválido."))
        if size <= 0 or size > self.max_size:
            raise ValidationError(
                gettext("O arquivo está vazio ou excede o limite permitido.")
            )
        ...
        if not any(header.startswith(signature) for signature in signatures):
            raise ValidationError(
                gettext("O conteúdo do arquivo não corresponde ao tipo informado.")
            )
...
def require_clean_malware_scan(upload: UploadedFile) -> None:
    ...
    if (
        ...
        raise ValidationError(
            gettext("A varredura de segurança do arquivo está indisponível.")
        )
    ...
    if completed.returncode != 0:
        raise ValidationError(
            gettext("O arquivo não foi aprovado na varredura de segurança.")
        )
