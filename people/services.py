...
def revoke_professional_credential(
    *,
    ...
    if credential.status == ProfessionalCredential.Status.REVOKED:
        raise ValidationError(_("Esta credencial já está revogada."))
    if not reason.strip():
...
def update_professional_profile(
    *,
    ...
    if not normalized_name or not normalized_email:
        raise ValidationError(_("Informe nome e e-mail profissional."))
    if category not in ProfessionalProfile.Category.values:
        raise ValidationError(_("Selecione uma categoria profissional válida."))
    ...
    if len(normalized_specialties) != len(specialties) or any(
        ...
        raise ValidationError(
            _("Informe especialidades técnicas válidas e sem repetição.")
        )
    ...
    if any(credential_parts) and not all(credential_parts):
        raise ValidationError(_("Informe conselho, número e jurisdição em conjunto."))
    ...
    if photo is not None:
        if (photo.size or 0) > 2 * 1024 * 1024:
            raise ValidationError(_("A foto excede o limite de 2 MB."))
        metadata = PrivateUploadPolicy().validate(photo)
        if metadata.detected_media_type not in {"image/png", "image/jpeg"}:
            raise ValidationError(_("A foto deve ser PNG ou JPEG."))
...
def register_patient_profile(
    *,
    ...
    if not normalized_name or not normalized_email:
        raise ValidationError(_("Informe nome e e-mail da pessoa paciente."))
    if gender and gender not in PatientProfile.Gender.values:
        raise ValidationError(_("Selecione uma opção de gênero válida."))
    if address and not address_purpose.strip():
        raise ValidationError(_("Informe a finalidade para registrar o endereço."))
    if emergency_contact and not emergency_contact_purpose.strip():
        raise ValidationError(_("Informe a finalidade para o contato de emergência."))
    ...
    if PatientProfile.infrastructure_objects.filter(
        ...
        raise ValidationError(
            _("Foi encontrada uma possível duplicidade nesta clínica.")
        )
...
def create_patient_care_relationship(
    *,
    ...
    if not function.strip():
        raise ValidationError(_("Informe a função do profissional no vínculo."))
...
def close_patient_care_relationship(
    *,
    ...
    if ended_on < relationship.valid_from:
        raise ValidationError(_("A data de encerramento não pode preceder o início."))
...
def transfer_patient_care_relationship(
    *,
    ...
    if relationship.therapist_id == new_therapist_id:
        raise ValidationError(_("O novo profissional deve ser diferente do atual."))
    if transferred_on < relationship.valid_from:
        raise ValidationError(_("A data de transferência não pode preceder o início."))
    original_valid_until = relationship.valid_until
    if original_valid_until is not None and transferred_on > original_valid_until:
        raise ValidationError(_("A data de transferência excede o término do vínculo."))
    roles = active_membership_roles_for_users(
        clinic_id=clinic_id,
