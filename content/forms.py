...
class EditorialContentForm(forms.Form):
    ...
    slug = forms.SlugField(max_length=160, label=_("Identificador"))
    title = forms.CharField(max_length=255, label=_("Título"))
    kind = forms.ChoiceField(choices=ContentKind.choices, label=_("Tipo"))
    language_code = forms.CharField(max_length=10, initial="pt-BR", label=_("Idioma"))
    audience = forms.ChoiceField(
        choices=(("patient", _("Paciente")), ("professional", _("Profissional"))),
        label=_("Público"),
    )
    categories = forms.CharField(required=False, max_length=200, label=_("Categorias"))
    tags = forms.CharField(required=False, max_length=200, label=_("Tags"))
    contraindications = forms.CharField(
        required=False,
        max_length=1000,
        label=_("Contraindicações"),
        widget=forms.Textarea(attrs={"rows": 3}),
    )
    source_reference = forms.CharField(
        required=False, max_length=255, label=_("Referência da fonte")
    )
    ...
    block_type = forms.MultipleChoiceField(
        ...
        choices=(
            ("heading", _("Título")),
            ("paragraph", _("Parágrafo")),
            ("list_item", _("Item de lista")),
        ),
        widget=HiddenMultipleChoice,
        label=_("Tipo de bloco"),
    )
    block_text = ValueListField(
        required=False,
        label=_("Texto do bloco"),
    )
    ...
    def clean(self) -> dict[str, Any]:
        ...
        if not self.has_blocks() and not (cleaned.get("body") or "").strip():
            self.add_error("body", _("Informe o conteúdo em blocos."))
        return cleaned
...
def _clean_taxonomy(value: object) -> str:
    ...
    if oversized:
        raise forms.ValidationError(_("Cada termo deve ter no máximo 64 caracteres."))
    return ",".join(terms)
...
class EditorialVersionForm(forms.Form):
    ...
    scheduled_for = forms.DateTimeField(required=False, label=_("Agendar publicação"))
...
class EditorialApprovalForm(forms.Form):
    opinion = forms.CharField(
        max_length=2000, label=_("Parecer"), widget=forms.Textarea(attrs={"rows": 4})
    )
    review_valid_days = forms.IntegerField(
        min_value=1, required=False, label=_("Validade da revisão em dias")
    )


class EditorialRollbackForm(forms.Form):
    target_version = forms.IntegerField(min_value=1, label=_("Versão de destino"))


class EditorialMediaForm(forms.Form):
    file = forms.FileField(label=_("Arquivo de mídia"))
