...
from .notification_copy import localized_notification_body
...
def retire_recommendation(
    *,
    ...
    if recommendation.status == "retired":
        raise ValidationError(_("Esta recomendação já está retirada."))
    if not reason.strip():
        raise ValidationError(_("Informe o motivo da retirada."))
    recommendation.status = "retired"
...
def notify_recommendation_change(
    *,
    ...
    languages = dict(
        user_model.objects.filter(pk__in=recipient_ids).values_list(
            "pk", "preferred_language"
        )
    )
    notifications = [
        ContentNotification.infrastructure_objects.create(
            ...
            recommendation=recommendation,
            body=localized_notification_body(
                kind=kind,
                language=languages.get(recipient_id),
            ),
        )
...
def recommend_content(
    *,
    ...
    if priority.strip() not in {"low", "normal", "high"}:
        raise ValidationError(_("Informe uma prioridade válida."))
    if not context.strip():
        raise ValidationError(_("Informe o contexto clínico da recomendação."))
    if valid_days is not None and valid_days <= 0:
        raise ValidationError(_("Informe um período de validade positivo."))
    credential = verified_professional_credential(clinic_id=clinic_id, actor=actor)
    lock_clinic_for_update(clinic_id=clinic_id)
    content = Content.infrastructure_objects.filter(
        pk=content_id, clinic_id=clinic_id
    ...
    if content.valid_until is not None and content.valid_until < timezone.localdate():
        raise ValidationError(
            _("Este conteúdo expirou e não pode mais ser recomendado.")
        )
    if (
        not ClinicMembership.objects.for_clinic(clinic_id)
    ...
    if patient_id is None and cohort_id is None:
        raise ValidationError(_("Informe o paciente ou a coorte destinatária."))
    valid_until = (
