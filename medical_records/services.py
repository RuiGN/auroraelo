...
def _compute_entry_hash(entry: MedicalRecordEntry) -> str:
    ...
    canonical = (
        f"{entry.title}|{entry.content}|{entry.objective_data}|{entry.current_version}"
    )
...
def create_record_addendum(
    *,
    ...
    existing_count = RecordAddendum.infrastructure_objects.filter(entry=entry).count()
    ...
    addendum.save(update_fields=["content_hash", "signed_at", "signed_by", "is_signed"])
